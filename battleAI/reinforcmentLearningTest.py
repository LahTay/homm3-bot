...
class Environment:
    def __init__(self, setOfCreatures, setOfObstacles):
        """
    ...
    def __possibleMovesFlying(self):
        """
        Algorithm:
    ...
    def __possibleMovesFlying1Hex(self, mob):
        ...
        if mob.type.speed >= 18:
            ...
            possibleMoves = self.__possibleMovesFlying()
    ...
    def choosePossibleMoves(self, mob: CreatureBox):
        """
    ...
    def moveCreature(self, hexField: tuple):
        """
    ...
    def prepareInputForNN(self, currPos):
        """
        ...
        for x in range(self.state.shape[0]):
            for y in range(self.state.shape[1]):
                ...
                # If hex is occupied by creature
                elif isinstance(self.state[x][y], CreatureBox):
                    features = self.state[x][y].returnFeatures()
                    # lets the bot know which unit is currently moving
                    if (x, y) == currPos:
                        inputNN[x][y][0] = 3/3
                    else:
                        inputNN[x][y][0] = 2/3
                ...
                # If hex is occupied by obstacle
                else:
                    inputNN[x][y][0] = 0/3
