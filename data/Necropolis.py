"""Script containing Necropolis city class"""
...
class Necropolis(City):
    def __init__(self,
                 name: str = "Necropolis",
                 mage_guild: MageGuild = Necropolis_Mage_Guild,
                 city_hall: CityHall = Necropolis_City_Hall,
                 fort: Fort = Necropolis_Fort,
                 marketplace: Marketplace = Necropolis_Marketplace,
                 tavern: Tavern = Necropolis_Tavern,
                 resource_silo: ResourceSilo = Necropolis_Rescource_Silo,
                 blacksmith: Building = Necropolis_Blacksmith,
                 t1: Habitat = Cursed_Temple,
                 t2: Habitat = Graveyard,
                 t3: Habitat = Tomb_of_Souls,
                 t4: Habitat = Estate,
                 t5: Habitat = Mausoleum,
                 t6: Habitat = Hall_of_Darkness,
                 t7: Habitat = Dragon_Vault,
                 cover: Building = Cover_of_Darkness,
                 amp: Building = Necromancy_Amplifier,
                 transformer: Building = Skeleton_Transformer,
                 graves: Building = Unearthed_Graves,
                 graal: bool = False,
                 upper_hero: Hero = Hero(0, "", "", 0, 0, 0, 0),
                 lower_hero: Hero = Hero(0, "", "", 0, 0, 0, 0),
                 owned_by: str = 'neutral'):
        """
        A class representing Necropolis city
        ...
        :param t1: 1st level habitat - Cursed temple
        :param t2: 2nd level habitat - Graveyard
        :param t3: 3rd level habitat - Tomb of souls
        :param t4: 4th level habitat - Estate
        :param t5: 5th level habitat - Mausaleum
        :param t6: 6th level habitat - Hall of Darkness
        :param t7: 7th level habitat - Dragon Vault
        :param cover: Cover of darness object
        :param amp: Necromancy amplifier object
        :param transformer: Skeleton transformer object
        :param graves: Unearthed graves object
        ...
        self.cover_of_darkness = cover
        self.necromancy_amplifier = amp
        self.skeleton_transformer = transformer
        self.unearthed_graves = graves
    ...
    def crop_building_names(self):
        ...
        print('Necromancy Amplifier')
        img_copy = img[453:453 + h, 982:982 + w]
        result = 'Necromancy Amplifier', super().check_color(img_copy)
        result_array.append(result)

        if result[1] == 'yellow':
            self.necromancy_amplifier.built = True

        # Cover of Darkness
        print('Cover of Darkness')
        img_copy = img[557:557 + h, 691:691 + w]
        result = 'Cover of Darkness', super().check_color(img_copy)
        result_array.append(result)

        if result[1] == 'yellow':
            self.cover_of_darkness.built = True

        # Skeleton Transformer
        print('Skeleton Transformer')
        img_copy = img[557:557 + h, 885:885 + w]
        result = 'Skeleton Transformer', super().check_color(img_copy)
        result_array.append(result)

        if result[1] == 'yellow':
            self.skeleton_transformer.built = True

        # Unearthed Graves
        print("Unearthed Graves")
        img_copy = img[557:557 + h, 1079:1079 + w]
        result = "Unearthed Graves", super().check_color(img_copy)
        result_array.append(result)

        if result[1] == 'yellow':
            self.unearthed_graves.built = True
